#!/usr/bin/env bash
# propagate-agent-teams-v3.sh
# Fleet rollout of the AGENT TEAM DOCTRINE v3 (2026-08-11):
#   - the orchestrator is the COORDINATOR: decompose+select, partition scope,
#     refine iteratively, route centrally (hub-and-spoke)
#   - subagents start BLANK: the Agent(...) brief is the only channel
#   - decomposition.md (repo root) written BEFORE the first spawn
#   - citations survive as structured data; parallel spawns; sign vs lock (hooks);
#     fixed-pipeline vs dynamic decomposition; attention dilution -> multi-pass;
#     session moves (resume / fork / fresh-start-with-summary)
#   - roster frontmatter aligned with the opus-first model policy (2026-08-08)
#
...
#
# Usage:
#   ./scripts/propagate-agent-teams-v3.sh --dry-run --verbose
#   ./scripts/propagate-agent-teams-v3.sh --only nicokaz
#   ./scripts/propagate-agent-teams-v3.sh --no-commit
#   ./scripts/propagate-agent-teams-v3.sh                # writes + commits
#
# Skip rules (same contract as propagate-model-routing.sh): Projects/_archived/,
...
VAULT="${OBSIDIAN_VAULT:-/Users/aleksejpravotorov/Desktop/My AI Knowledge Base}"
...
SUBAGENTS="frontend backend devops skeptic qa researcher"
COMMANDS="build-with-agent-team deploy prime"

COMMIT_MSG="chore(fleet): agent-team doctrine v3 - coordinator, blank-slate briefs, decomposition log

Hub-and-spoke routing, isolation principle (the brief is the only channel),
mandatory decomposition.md, structured citation passing, sign-vs-lock
enforcement, fixed-vs-dynamic decomposition, attention-dilution multi-pass,
session moves. Roster frontmatter aligned with the opus-first model policy.
...
# ── preflight: canonical sources must already carry doctrine v3 ─────────────
preflight() {
  local ok=1 f name
  if ! /usr/bin/grep -q "Coordinator doctrine" "$CANON_AGENTS_MD"; then
    echo "PREFLIGHT FAIL: $CANON_AGENTS_MD lacks the 'Coordinator doctrine' block" >&2; ok=0
  fi
  if ! /usr/bin/grep -q "opus-first" "$CANON_AGENTS_MD"; then
    echo "PREFLIGHT FAIL: $CANON_AGENTS_MD lacks the opus-first model policy" >&2; ok=0
  fi
  ...
  for name in $ROSTER; do
    ...
    if ! /usr/bin/grep -q "^model: opus" "$f"; then
      echo "PREFLIGHT FAIL: $f lacks 'model: opus' frontmatter (opus-first policy)" >&2; ok=0
    fi
    ...
    if /usr/bin/grep -q $'\xe2\x80\x94' "$f"; then
      echo "PREFLIGHT FAIL: $f contains em-dashes (U+2014)" >&2; ok=0
    fi
  done
  # every spoke carries the blank-slate contract; the hub carries the four jobs
  for name in $SUBAGENTS; do
    if ! /usr/bin/grep -q "SUBAGENT CONTRACT" "$CANON_AGENT_DIR/$name.md"; then
      echo "PREFLIGHT FAIL: $CANON_AGENT_DIR/$name.md lacks the SUBAGENT CONTRACT block" >&2; ok=0
    fi
  done
  if ! /usr/bin/grep -q "four jobs" "$CANON_AGENT_DIR/lead.md"; then
    echo "PREFLIGHT FAIL: $CANON_AGENT_DIR/lead.md lacks the coordinator's four jobs" >&2; ok=0
  fi
  # commands
  f="$CANON_CMD_DIR/build-with-agent-team.md"
  if [ ! -f "$f" ]; then
    echo "PREFLIGHT FAIL: missing $f" >&2; ok=0
  else
    for token in "COORDINATOR" "decomposition.md" "hub-and-spoke" "blank"; do
      if ! /usr/bin/grep -qi -- "$token" "$f"; then
        echo "PREFLIGHT FAIL: $f lacks '$token'" >&2; ok=0
      fi
    done
    if ! /usr/bin/grep -q "^allowed-tools:.*Agent" "$f"; then
      echo "PREFLIGHT FAIL: $f allowed-tools must include Agent or delegation silently never happens" >&2; ok=0
    fi
  fi
  if ! /usr/bin/grep -q "cc-update.sh" "$CANON_CMD_DIR/prime.md"; then
    echo "PREFLIGHT FAIL: $CANON_CMD_DIR/prime.md lacks the cc-setup freshness gate" >&2; ok=0
  fi
  for name in $COMMANDS; do
    if [ ! -f "$CANON_CMD_DIR/$name.md" ]; then
      echo "PREFLIGHT FAIL: missing canonical command $CANON_CMD_DIR/$name.md" >&2; ok=0
    fi
  done
  [ "$ok" = "1" ] || exit 2
}
